...
export class DialogEditKillerComponent implements OnInit {
  killer: killers;
  editForm: FormGroup;
  constructor(public dialogRef: MatDialogRef<DialogEditKillerComponent>, @Inject(MAT_DIALOG_DATA) public data: any,
              private killersService: KillersService, private fb: FormBuilder) { }
  ...
  updateKiller(killerId: number) {
    this.killersService.updateKiller(killerId, this.editForm.value).subscribe(data => console.log(data), error => console.log(error));
    this.dialogRef.close();
  }
}
